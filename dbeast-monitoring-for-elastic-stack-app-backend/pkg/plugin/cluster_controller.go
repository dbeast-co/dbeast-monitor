...
func (a *App) AddNewClusterContainerHandler(response http.ResponseWriter, request *http.Request) {
	a.AddNewClusterHandler(response, request, true)
	return
}

func (a *App) AddNewClusterOnPremHandler(response http.ResponseWriter, request *http.Request) {
	a.AddNewClusterHandler(response, request, false)
	return
}

func (a *App) AddNewClusterHandler(response http.ResponseWriter, request *http.Request, isContainer bool) {
	ctxLogger := log.DefaultLogger.FromContext(request.Context())
	ctxLogger.Info("Got request for the new cluster save")
	response.Header().Add("Content-Type", "application/json")
	...
	if isContainer {
		_, done := a.AddLogstashFilesContainer(response, err, clusterId, ctxLogger, cluster)
		if done {
			return
		}
	}
	//End block for the container version
...
func (a *App) AddLogstashFilesContainer(response http.ResponseWriter, err error, clusterId string, ctxLogger log.Logger, cluster Project) (error, bool) {
