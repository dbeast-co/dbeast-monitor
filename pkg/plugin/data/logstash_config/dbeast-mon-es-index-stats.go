...
const dbeastMonEsIndexStatsContent string = `
input{
  http_poller {
    id => "Input-get-cat-indices-from-ES" 
    urls => {
      stats1 => "<PROD_HOST>/_cat/indices?bytes=b&time=ms&format=json&h=*&health=green&expand_wildcards=all"
      stats2 => "<PROD_HOST>/_cat/indices?bytes=b&time=ms&format=json&h=*&health=yellow&expand_wildcards=all"
    }
	user => "<PROD_USER>"
...
func init() {
	dataWarehouse.AppendLogstashConfig("dbeast-mon-es-index-stats", dbeastMonEsIndexStatsContent)
}
