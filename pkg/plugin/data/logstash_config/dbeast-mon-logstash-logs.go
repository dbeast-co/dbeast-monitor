...
const dbeastMonLogstashLogsContent string = `
input {
  file {
...
filter {
  ...
  elseif "DecoderException" in [logstash][message] or "Error decoding data" in [logstash][message] {
    mutate { 
...
func init() {
	dataWarehouse.AppendLogstashConfig("dbeast-mon-logstash-logs", dbeastMonLogstashLogsContent)
}
