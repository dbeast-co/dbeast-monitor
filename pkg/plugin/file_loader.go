...
func testEmbed() {
	...
	folderMaps := make(map[string]map[string]string)

	err := fs.WalkDir(dataFiles, "public/data", func(path string, d fs.DirEntry, err error) error {
		if err != nil {
			return err
		}

		if !d.IsDir() {
			dir := filepath.Base(filepath.Dir(path))                 // folder name
			name := strings.TrimSuffix(filepath.Base(path), ".json") // file name without .json

			// Ensure the map for this folder exists
			if _, exists := folderMaps[dir]; !exists {
				folderMaps[dir] = make(map[string]string)
			}
			...
			// Store content
			folderMaps[dir][name] = string(content)
		}
		return nil
	})
	...
	// Example: print all maps
	for folder, files := range folderMaps {
		log.DefaultLogger.Info("Folder:", folder)
		for name, content := range files {
			log.DefaultLogger.Info("Content: ", name, content)
		}
	}
}
...
//func LoadFolderFromFS(folder string)  {
//	err := fs.WalkDir(dataFiles, folder, func(path string, d fs.DirEntry, err error) error {
//		if err != nil {
//			return err
//		}
//
...
//
//			// Read file content
//			log.DefaultLogger.Info("Reading file: " + path)
//			content, err := dataFiles.ReadFile(path)
//			if err != nil {
//				return err
//			} else {
